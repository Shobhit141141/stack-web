...
export const env = {
  ...
  URL_FETCH_MAX_BYTES: Math.min(
    50 * 1024 * 1024,
    Math.max(1024, Number(process.env.URL_FETCH_MAX_BYTES) || 10 * 1024 * 1024),
  ),

  URL_FETCH_TIMEOUT_MS: Math.min(
    120_000,
    Math.max(5_000, Number(process.env.URL_FETCH_TIMEOUT_MS) || 30_000),
  ),
