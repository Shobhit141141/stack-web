...
const STACK_FILE_TOOL_SESSION_HINT =
  'Stack session hint: Any question about which files the user has, file names, uploads, or what is in their workspace is in scope. You must invoke the file-search server tool for those requests and answer only from tool results. Preserve the user intent in tool arguments: keep important keywords, keep workspace constraints, and do not rewrite to generic short queries. If the user says "my workspace", keep that scope and use workspace context from metadata when available. For download/copy/delete/move/rename requests, call stackFileAction directly and do not call apiCalls for those commands. For move, pass workspaceName or workspaceId; for rename, pass newName. Do not refuse, and do not tell the user to open the app or check a file list manually instead of using the tool. Responses may end with a machine block between <<<STACK_META>>> and <<<END_STACK_META>>>; never read that block or anything inside it aloud—it is for the app UI only.'
